...
import { InventoryBrand, InventoryCategory, InventoryPagination, InventoryProduct, InventoryTag, InventoryVendor, SuAdmin } from './add-suadmin.types';
import { APP_CONFIG } from 'app/core/config/app-config';

@Injectable({ providedIn: 'root' })
export class AddsuadminService {
    // Private
    ...
    private _usuarios: BehaviorSubject<SuAdmin[] | null> = new BehaviorSubject<SuAdmin[] | null>(null);
    ...
    get usuarios$(): Observable<SuAdmin[]> {
        return this._usuarios.asObservable();
    }


    // Obtener todos los superadmins
    getUsuarios(): Observable<SuAdmin[]> {
        return this._httpClient.get<{ message: string, data: SuAdmin[] }>(`${this.apiUrl}superadmin/data`)
            .pipe(
    ...
    getUsuarioById(id: string): Observable<SuAdmin> {
        return this._httpClient.get<{ message: string, user: SuAdmin }>(`${this.apiUrl}superadmin/suadmin/${id}`)
            .pipe(
    ...
    // Crear superadmin
    // add-suadmin.service.ts
    createUsuario(formData: FormData): Observable<SuAdmin> {
        return this.usuarios$.pipe(
            take(1),
            switchMap(usuarios =>
                this._httpClient.post<{ message: string, user: SuAdmin }>(
                    `${this.apiUrl}superadmin/suadmin`,
                    formData 
                ).pipe(
                    tap(response => {
                        this._usuarios.next([...(usuarios || []), response.user]);
                    }),
                    map(response => response.user)
                )
            )
    ...
    updateUsuario(id: string, usuario: { name: string, email: string, password?: string }): Observable<SuAdmin> {
        return this.usuarios$.pipe(
            take(1),
            switchMap(usuarios =>
                this._httpClient.put<{ message: string, user: SuAdmin }>(`${this.apiUrl}superadmin/suadmin/${id}`, usuario)
                    .pipe(
                        tap(response => {
    ...
    deleteUsuario(id: string): Observable<boolean> {
        return this.usuarios$.pipe(
            take(1),
            switchMap(usuarios =>
                this._httpClient.delete<{ message: string, user: SuAdmin }>(`${this.apiUrl}superadmin/suadmin/${id}`)
                    .pipe(
                        tap(() => {
