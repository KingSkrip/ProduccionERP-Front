...
@Component({
  selector: 'tabs-tejido-revisadojc',
...
export class TejidoRevisadoTabComponent implements OnInit, OnDestroy {
  ...
  private aplicarFiltrosLocales(filtros: any): void {
    ...
    this.datosFiltrados = this.datos.filter((item) => {
      ...
      const coincideDepto =
        !deptoSeleccionado || (item.departamento && item.departamento === deptoSeleccionado);
  ...
  cargarRevisadoTejido(fechaInicio?: Date | null, fechaFin?: Date | null): void {
    ...
    this._reportService
      .getRevisadoTejido(fechaInicio || undefined, fechaFin || undefined)
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe({
        next: (response) => {
          this.datos = response;
  ...
  calcularTotalPesoRevisado(): number {
    return this.datosFiltrados.reduce((total, item) => total + (parseFloat(item.TOTAL_RV) || 0), 0);
  }

  calcularTotalPiezasRevisado(): number {
    return this.datosFiltrados.reduce((total, item) => total + (parseFloat(item.PIEZAS) || 0), 0);
  }
