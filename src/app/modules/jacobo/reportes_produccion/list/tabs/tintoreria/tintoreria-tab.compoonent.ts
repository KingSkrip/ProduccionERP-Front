...
export class TintoreriaTabComponent implements OnInit, OnDestroy {
  datos: any[] = [];
  datosFiltrados: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this._sharedDataService.recargarDatos$
      ...
      .subscribe(() => {
        ...
        this.cargarTintoreria(filtros.fechaInicio, filtros.fechaFin);
  ...
  cargarTintoreria(fechaInicio?: Date | null, fechaFin?: Date | null): void {
    this.loading = true;
    this._cd.markForCheck();

    this._reportService.getTintoreria(fechaInicio || undefined, fechaFin || undefined).subscribe({
      next: (data) => {
        this.datos = data;
        ...
        this._sharedDataService.actualizarTintoreria(data);
      ...
      error: () => {
        this.loading = false;
        this._snackBar.open('Error al cargar Tintorería', 'Cerrar', { duration: 3000 });
        this._cd.markForCheck();
      },
    });
  }
  ...
  calcularCantidadTotal(): number {
    return this.datosAgrupados.reduce((total, grupo) => total + grupo.cantidadTotal, 0);
  }
