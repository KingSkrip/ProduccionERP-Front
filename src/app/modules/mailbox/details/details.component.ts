...
export class MailboxDetailsComponent implements OnInit, OnDestroy {
  ...
  toggleimportantes(): void {
    ...
    if (!this.mail.importantes) {
      // If the current activated route has a filter parameter and it equals to the 'importantes'...
      if (
        this._activatedRoute.snapshot.paramMap.get('filter') &&
        this._activatedRoute.snapshot.paramMap.get('filter') === 'importantes'
      ) {
        // Navigate to the parent
        this._router.navigate(['./'], {
          relativeTo: this._activatedRoute.parent,
  ...
  toggleStar(): void {
    ...
    // Update the mail on the server
    this._mailboxService.updateMail(this.mail.id, { destacados: this.mail.destacados }).subscribe();
    ...
    if (!this.mail.destacados) {
      ...
      if (
        this._activatedRoute.snapshot.paramMap.get('filter') &&
        this._activatedRoute.snapshot.paramMap.get('filter') === 'destacados'
      ) {
        // Navigate to the parent
        this._router.navigate(['./'], {
          relativeTo: this._activatedRoute.parent,
