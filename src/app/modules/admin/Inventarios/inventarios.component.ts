...
export class InventariosComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  aplicarFiltro(): void {
    ...
    const itemsFiltrados = !term
      ...
      : this.items.filter((item) =>
          [item['CVE ART'], item.ARTICULO, item.CLIENTE, item.AGENTE, item.PEDIDO, item.OP]
            .filter(Boolean)
  ...
  get hayFiltrosRollosActivos(): boolean {
    return !!(this.busquedaRollos || this.filtroTipo || this.filtroColor || this.filtroAgente);
  }
  ...
  private buscarRolloPorCodigo(codigo: string): void {
    ...
    this.inventariosService
      ...
      .subscribe({
        ...
        error: (err) => {
          this.escaneando = false;
          this.errorEscaner = err?.error?.message ?? 'No se encontró ningún rollo con ese código.';
          this.cdr.markForCheck();
        },
      });
  }
  ...
  private readonly camposPesado: { label: string; key: string }[] = [
    ...this.camposComunes.map((c) => (c.key === 'FECHA' ? { ...c, label: 'Fecha de Orden' } : c)),
    ...
    { label: 'Cantidad en el baño', key: 'CANTENT' },
  ...
  get pesoNetoClienteTotal(): number {
    return this.rollosFiltrados.reduce((s, item) => s + (Number(item['PESO NETO']) || 0), 0);
  }
}
